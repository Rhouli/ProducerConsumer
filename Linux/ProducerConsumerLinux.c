 ...
 *   #define TTL_THREADS (NUM_PRODUCER+NUM_CONSUMER)
 *   #include <pthread.h>
 *   #include <semaphore.h>
 *
 *   sem_t empty;
 *   sem_t full;
 *   pthread_mutext_t count_mutex;
 *   int conPos = 0
 *   int proPos = 0
 *
 *   Main:
 ...
 *	pthread_attr_t attr;
 *	
 *	// initalize mutex and conditon variable objects
 *	pthread_mutex_init(&count_mutex, NULL);
 *	sem_init(&full, NULL);
 *	sem_init(&empty, NULL);
 *
 *	// Create thread sin joinable state
 *	pthread_attr_init(&attr);
 *	pthread_attr_setdetachstate(&attr, PTHREAD_CREATE_JOINABLE);
 *	// Create Producer thread(s)
 *	for(i = 0; i < NUM_PRODUCER; i++)
 *		pthread_create(&threads[i], &attr, Producer, (void *)i);
 *	// Create Consumer thread(s)
 *	for( ; i < TTL_THREADS; i++)
 *	     pthread_create(&threads[i], &attr, Consumer, (void *)i);
 *
 *	// Wait for all threads
 *	for(i=0; i < TTL_THREADS; i++)
 *		 pthread_join(threads[i], NULL);
 *		 
 *	// Clean and exit
 *	pthread_attr_destroy(&attr);
 *	pthread_mutex_destroy(&mutex);
 *	sem_destroy(&full, &empty)
 *	pthread_exit(NULL);
 *
 *   Produce:
 *	create something
 ...
 *	    // Declare producer is out of critical section
